...
(function(){
    ...
    let avatarImg, avatarBox, charClassSpan, charLevelSpan, expBarFill, expText, goldSpan, diamondSpan, msgDiv;
    ...
    const EQUIPMENT = {
        ...
        // 帝套 (橙色品质，全职业通用，强化币购买)
        '帝霜冰剑': { name: '帝霜冰剑', type: EQUIPMENT_TYPE.WEAPON, rarity: EQUIPMENT_RARITY.ORANGE, class: 'all', price: 50, currency: 'enhanceCoin', props: { attack: 150, criticalRate: 0.15 }, growth: { attack: 15, criticalRate: 0.015 } },
        '帝鬼魔盔': { name: '帝鬼魔盔', type: EQUIPMENT_TYPE.HEAD, rarity: EQUIPMENT_RARITY.ORANGE, class: 'all', price: 30, currency: 'enhanceCoin', props: { defense: 50, maxHp: 400, criticalRate: 0.05 }, growth: { defense: 5, maxHp: 40, criticalRate: 0.005 } },
        '帝冥骨甲': { name: '帝冥骨甲', type: EQUIPMENT_TYPE.CHEST, rarity: EQUIPMENT_RARITY.ORANGE, class: 'all', price: 35, currency: 'enhanceCoin', props: { defense: 70, maxHp: 600 }, growth: { defense: 7, maxHp: 60 } },
        '帝棘之裤': { name: '帝棘之裤', type: EQUIPMENT_TYPE.LEGS, rarity: EQUIPMENT_RARITY.ORANGE, class: 'all', price: 25, currency: 'enhanceCoin', props: { defense: 40, attack: 30 }, growth: { defense: 4, attack: 3 } },
        '帝炎之靴': { name: '帝炎之靴', type: EQUIPMENT_TYPE.BOOTS, rarity: EQUIPMENT_RARITY.ORANGE, class: 'all', price: 20, currency: 'enhanceCoin', props: { speed: 30, attack: 20 }, growth: { speed: 3, attack: 2 } },
        '帝渊之眼': { name: '帝渊之眼', type: EQUIPMENT_TYPE.ACCESSORY, rarity: EQUIPMENT_RARITY.ORANGE, class: 'all', price: 40, currency: 'enhanceCoin', props: { attack: 50, magic: 50, criticalRate: 0.1 }, growth: { attack: 5, magic: 5, criticalRate: 0.01 } },
    ...
    function updateShopCurrency() {
        ...
        const enhanceSpan = document.querySelector('.currency-enhance');
        
        if (goldSpan) goldSpan.textContent = `💰 ${userData.gold || 0}`;
        if (diamondSpan) diamondSpan.textContent = `💎 ${userData.diamond || 0}`;
        if (enhanceSpan) enhanceSpan.textContent = `⚙️ ${userData.enhanceCoin || 0}`;
    }
        ...
        if (tab === 'item') {
            ...
            switch(tab) {
                case 'class':
                    ...
                    break;
                case 'traveler':
                    filteredEquipment = filteredEquipment.filter(e => e.class === 'all' && e.rarity === EQUIPMENT_RARITY.BLUE);
                    break;
                case 'emperor':
                    filteredEquipment = filteredEquipment.filter(e => e.class === 'all' && e.rarity === EQUIPMENT_RARITY.ORANGE);
                    break;
                case 'creator':
                    filteredEquipment = filteredEquipment.filter(e => e.class === 'all' && e.rarity === EQUIPMENT_RARITY.RED);
                    break;
            }
            ...
            filteredEquipment.forEach(equip => {
                ...
                html += `
                    <div class="shop-item-card rarity-${equip.rarity} ${isSoldOut ? 'sold-out' : ''}" data-equipment-name="${equip.name}">
                        <img src="../image/equipment/${equip.name}.png" alt="${equip.name}" class="shop-item-img" onerror="this.src='../image/equipment/新手铁剑.png'">
                        <div class="shop-item-name">${equip.name}</div>
                        ${isSoldOut ? 
                            '<div class="shop-item-soldout-text">非卖品</div>' : 
                            `<div class="shop-item-price ${currency === 'enhanceCoin' ? 'enhance-coin' : ''}">${currency === 'enhanceCoin' ? '⚙️' : '💰'} ${price}</div>`
                        }
                    </div>
                `;
            });
    ...
    function handleBuyEquipment(equip) {
        ...
        if (currency === 'enhanceCoin' && (userData.enhanceCoin || 0) < price) {
            showMessage('强化币不足！');
            return;
        }
        ...
        if (currency === 'gold') {
            userData.gold -= price;
        } else if (currency === 'enhanceCoin') {
            userData.enhanceCoin = (userData.enhanceCoin || 0) - price;
        }
    ...
    function dismantleEquipment(equip) {
        ...
        if (currency === 'gold') {
            ...
            userData.enhanceCoin = (userData.enhanceCoin || 0) + totalReturn;
        }
        ...
        saveAccounts();
        showMessage(`分解成功！获得${currency === 'enhanceCoin' ? '⚙️' : '💰'}${totalReturn}`);
        refreshVillageUI();
    }
    ...
    function battleWin(isTame) {
        ...
        try {
            ...
            if (battleState.challengeMode === 'endless') {
                ...
                return;
            } else if (battleState.challengeMode === 'bossEvolution') {
